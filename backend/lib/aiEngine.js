...
function shapeSpec(input) {
  ...
  if (boardKind === 'quiz') {
    ...
    const plainTask = `Produce exactly ${count} ${isMcqGap ? 'multiple-choice gap-fill sentences (one blank, 4 options)' : 'multiple-choice questions (4 options)'} at ${level} level, each concretely about "${topic}" - a specific fact, phrase or scenario tied to it, never a generic sentence that could belong to any topic. No two items may test the same point. Each has 4 options and "answer" equal to the correct option text; every distractor must be plausible (same topic and register as the answer), not an obviously-wrong filler word.`;
    return {
  ...
  if (toolId === 'worksheet-builder') {
    return {
      task: `${head} Build an interactive ESL worksheet at ${level} level on the topic: "${topic}". Every stem sentence must be concretely about "${topic}" - a specific fact, scenario or detail, never a generic sentence that could belong to any worksheet on any topic. Distractor options must be plausible wrong answers (same part of speech / same topic register), not random unrelated words. Return exactly 3 parts in this order: (1) type "multiple_choice" - 6 items, each with a stem sentence containing a blank "___" and 4 options A-D, answer = index 0-3; (2) type "fill_blank" - 6 items with stem sentences containing "___", a word_bank array of 8 words, answer = correct word; (3) type "matching" - 6 items, each item has a sentence stem and 3 options, answer = index of correct ending. Keep language natural and ${level}-appropriate. All answers must be clearly correct.${context}`,
