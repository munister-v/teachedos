...
pool.query(`ALTER TABLE users ADD COLUMN IF NOT EXISTS onboarded BOOLEAN DEFAULT false`).catch(() => {});
...
async function requireAuth(req, res, next) {
  ...
  try {
    ...
    const { rows } = await pool.query(
      'SELECT id, email, name, role, avatar, plan, plan_status, billing_cycle, plan_started_at, plan_expires_at, plan_source, meeting_url, zoom_url, timezone, timezone_mode, onboarded, created_at FROM users WHERE id = $1',
      [payload.sub]
    );
