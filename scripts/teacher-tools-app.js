...
function generate(){if(!activeTool)return;const m=activeTool.mode;const n=count();const src=get('source')||SAMPLE_TEXT;const voc=get('vocab')||SAMPLE_VOCAB;const items=itemsForCount(voc,n);let out={title:activeTool.title,type:m,text:'',cards:[],gameType:activeTool.game||null,gameContent:null,level:level(),tags:[activeTool.cat,topic()]};
  ...
  else if(m==='worksheet'){const ws=wordsForCount(voc,n);const t=topic()||'English';const lv=level()||'B1';const defs=makeDefinitions(items).slice(0,Math.min(ws.length,8));const wsCards=[{title:'📝 Student information',text:'Student name: ____________________   Date: __________\nClass: ____________________   Teacher: __________\nLevel: '+lv+'   Topic: '+t},{title:'A. Vocabulary task',text:'Match the words with their meanings:\n\n'+ws.slice(0,6).map((w,i)=>`${i+1}. ${w}  ___________________________`).join('\n')+'\n\nWord bank: '+(ws.slice(0,6).sort(()=>Math.random()-.5).join(' · ')||'(vocabulary)')},{title:'B. Reading comprehension',text:'Read the text carefully and answer:\n\n1. What is the main idea? _________________________________ \n2. Give two supporting details:\n   a) _______________________________________\n   b) _______________________________________\n3. Find a word in the text that means: ___________________'},{title:'C. Language practice',text:'Complete the sentences using the target vocabulary:\n\n'+ws.slice(0,5).map((w,i)=>`${i+1}. _____ is important because _____________________________________.`).join('\n')},{title:'D. Speaking / writing task',text:'Choose ONE task:\n\n🗣 SPEAKING: Discuss with a partner — how does '+t.toLowerCase()+' affect everyday life? Use at least 3 target words.\n\n✍️ WRITING: Write 80–100 words about '+t.toLowerCase()+'. Include your opinion and at least 4 target words.'},{title:'🔑 Answer key (teacher use)',text:'Vocabulary:\n'+defs.map((p,i)=>`${i+1}. ${p.a} — ${p.b}`).join('\n')+'\n\n📌 Differentiation:\n• Support: give sentence starters for the writing task\n• Extension: write 2 follow-up questions for a partner\n• Time: 45–60 min total (reading included)'}];out.text=wsCards.map(c=>c.title+'\n'+c.text).join('\n\n');out.struct={boardKind:'cards',cards:wsCards,items:null,questions:null};}
  else if(m==='homework'){const ws=wordsForCount(voc,n);const t=topic()||'English';const lv=level()||'B1';const hwCards=[{title:'📚 Homework brief',text:'Topic: '+t+' · Level: '+lv+'\nEstimated time: 25–35 minutes\nDue date: ____________________\n\nComplete all four tasks. Quality matters more than length.'},{title:'Task 1 — Vocabulary review',text:'Words to practise: '+(ws.join(', ')||'target vocabulary')+'.\n\n★ Write one personal sentence with each word.\n★ Mark the word you find most useful.\n★ Write one question using any of the words.'},{title:'Task 2 — Reading / listening review',text:'Return to the lesson text or audio.\n\n• Write 5 key ideas in your own words.\n• Underline 3 pieces of language you want to use yourself.\n• Note one thing you are still unsure about.'},{title:'Task 3 — Speaking preparation',text:'Prepare a 60-second spoken answer:\n\n"What is your opinion about '+t.toLowerCase()+'?"\n\nStructure: main point → one example → recommendation.\n💡 Record yourself, listen back, improve one sentence.'},{title:'✅ Self-check before submitting',text:'☐ I used target vocabulary at least 3 times.\n☐ My sentences are complete and clear.\n☐ I can explain my answers without reading every word.\n☐ I checked my spelling and punctuation.\n\nMy strongest sentence: ________________________________\nOne thing to improve next time: ________________________________'}];out.text=hwCards.map(c=>c.title+'\n'+c.text).join('\n\n');out.struct={boardKind:'cards',cards:hwCards,items:null,questions:null};}
  ...
  else if(m==='grammar-rules'){const t=topic()||'Grammar point';const lv=level()||'B1';const grCards=[{title:'📏 Grammar focus: '+t,text:'Level: '+lv+'\n\n📌 FORM\nAffirmative: Subject + [verb form] + ...\nNegative: Subject + [aux] + not + [base form] + ...\nQuestion: [Aux] + Subject + [base form] + ...?\n\n📌 MEANING & USE\nWe use '+t.toLowerCase()+' when we want to express:\n• Situation 1: ...\n• Situation 2: ...'},{title:'✅ Correct examples',text:'Study these examples carefully:\n\n1. ...\n2. ...\n3. ...\n\n💡 Notice: which words signal that '+t.toLowerCase()+' is needed?'},{title:'❌ Common mistakes to avoid',text:'Mistake 1:\n   ✗ Wrong: ________________________________\n   ✓ Right: ________________________________\n\nMistake 2:\n   ✗ Wrong: ________________________________\n   ✓ Right: ________________________________\n\nMistake 3:\n   ✗ Wrong: ________________________________\n   ✓ Right: ________________________________'},{title:'✏️ Practice exercises',text:'A. Complete the sentences:\n1. She _____ (never / visit) Paris. → _______________\n2. They _____ (finish) the project yet? → _______________\n3. I _____ (study) English for five years. → _______________\n\nB. Write 3 original sentences using '+t.toLowerCase()+'.'},{title:'🗣 Speaking & review task',text:'Discuss with a partner:\n\n1. When did you last use '+t.toLowerCase()+' in real conversation?\n2. What makes it difficult? What helps?\n3. Give your partner one example sentence → ask them to identify the tense/form.\n\n💡 Tip: the best way to learn grammar is to use it immediately in real sentences.'}];out.text=grCards.map(c=>c.title+'\n'+c.text).join('\n\n');out.struct={boardKind:'cards',cards:grCards,items:null,questions:null};}
  ...
  else if(m==='dialogue'){const tp=topic();const tl=tp.toLowerCase();const lv=level()||'B1';const ws=wordsForCount(voc,4).join(', ')||'four target words';const dlgCards=[{title:'🗣 Role-play scenario',text:`Topic: ${tp} · Level: ${lv}\n\nA: I need some advice about ${tl}.\nB: Sure. What exactly happened?\nA: I think the main problem is vocabulary and confidence.\nB: Let\'s make a simple plan and practise it step by step.`},{title:'✏️ Task: Continue the dialogue',text:`Write 6 more lines for A and B.\nMake sure to include these words: ${ws}.\n\nA: _______________\nB: _______________\nA: _______________\nB: _______________\nA: _______________\nB: _______________`},{title:'🔑 Useful language',text:`Giving advice:\n• "Have you tried…?"\n• "One thing that helps me is…"\n• "I'd suggest…"\n• "Why don't you…?"\n\nResponding:\n• "That\'s a good idea."\n• "I\'m not sure about that because…"\n• "I\'ll definitely try…"`}];out.text=dlgCards.map(c=>c.title+'\n'+c.text).join('\n\n');out.struct={boardKind:'cards',cards:dlgCards,items:null,questions:null};}
  ...
  else if(m==='four-opinions'){const t=topic().toLowerCase();const people=['Anna (strongly agrees)','Ben (partly agrees)','Carla (disagrees)','David (not sure / neutral)'];const stances=[`thinks ${t} is clearly a good thing and gives one strong reason.`,`sees some benefits of ${t} but also points out one problem.`,`believes ${t} causes more harm than good.`,`cannot decide and asks two questions about ${t}.`];const foCards=[...people.map((p,i)=>({title:'💬 '+p,text:p.split(' (')[0]+' '+stances[i]+'\n\nWrite this person\'s opinion in 1–2 sentences:\n"_______________"'})),{title:'✍️ Your response',text:'Whose opinion is closest to yours and why?\n\nWrite 80-120 words using:\n• I think / believe / feel that…\n• In my opinion…\n• I agree with [name] because…\n• On the other hand…\n\nMy response:\n_______________'}];out.text=`Four Opinions: ${topic()} (${level()})\n\n`+people.map((p,i)=>`${p}: ${stances[i]}`).join('\n\n')+`\n\nWriting task: Whose opinion is closest to yours and why? Write 80-120 words.`;out.struct={boardKind:'cards',cards:foCards,items:null,questions:null};}
  ...
  else if(m==='essay-topics'){const t=topic();const frames=[`To what extent do you agree that ${t.toLowerCase()} improves our lives?`,`Discuss the advantages and disadvantages of ${t.toLowerCase()}.`,`Some people think ${t.toLowerCase()} is overrated. Do you agree?`,`How has ${t.toLowerCase()} changed in the last 20 years?`,`Should ${t.toLowerCase()} be encouraged at school? Why or why not?`,`"${t} is more of a problem than a solution." Discuss.`,`What are the most important effects of ${t.toLowerCase()} on society?`,`Compare two different views about ${t.toLowerCase()}.`];const qs=repeatToCount(frames,n);out.text=`Essay Topics: ${topic()} (${level()})\n\n`+qs.map((q,i)=>`${i+1}. ${q}`).join('\n')+`\n\nEach essay: 4 paragraphs, 180-250 words, with a clear thesis and examples.`;out.struct={boardKind:'quiz',questions:qs.map(q=>({type:'open',text:q+'\n\n📝 Format: 4 paragraphs · 180-250 words · clear thesis · at least 2 examples',answer:'',points:1})),items:null,cards:null};}
  else if(m==='lead-in'){const t=topic().toLowerCase();const acts=[`Show the word "${topic()}" and ask: what comes to mind?`,`Brainstorm: students list 5 words connected to ${t} in 60 seconds.`,`Quick poll: agree or disagree — "${topic()} is important to me."`,`Pair talk: share one personal experience with ${t}.`,`Guess the topic: give three clues about ${t} and let students guess.`,`Mini-ranking: order three aspects of ${t} from most to least interesting.`];const as=repeatToCount(acts,n);out.text=`Lead-in Activities: ${topic()} (${level()})\n\n`+as.map((a,i)=>`${i+1}. ${a}`).join('\n');out.struct={boardKind:'quiz',questions:as.map(a=>({type:'open',text:a,answer:'',points:1})),items:null,cards:null};}
  else if(m==='facts'){const t=topic().toLowerCase();const frames=[`Did you know that ${t} affects millions of people every day?`,`Many experts say ${t} has changed a lot in the last 50 years.`,`There is a surprising connection between ${t} and everyday habits.`,`The word for "${topic()}" exists in almost every language.`,`Research suggests that small changes in ${t} can have big effects.`,`${topic()} has a longer history than most people think.`];const fs=repeatToCount(frames,n);out.text=`Interesting Facts: ${topic()} (${level()})\n\n`+fs.map((f,i)=>`${i+1}. ${f}`).join('\n')+`\n\nTeacher note: these are discussion starters — ask students to check which could be true and find one real fact about ${t} for homework.`;out.struct={boardKind:'quiz',questions:fs.map(f=>({type:'open',text:f+'\n\n💬 Could this be true? Discuss. Then find one real fact about the topic for homework.',answer:'',points:1})),items:null,cards:null};}
  else if(m==='pros-cons'){const items=itemsForCount(voc,8);const t=topic().toLowerCase();const k=Math.min(Math.max(3,Math.ceil(n/2)),6);const proFrames=[w=>`Developing ${w} significantly improves outcomes in ${t}.`,w=>`Good ${w} helps learners communicate more confidently.`,w=>`${capitalize(w)} creates real opportunities for growth in ${t}.`,w=>`Practising ${w} consistently leads to faster, measurable progress.`,w=>`Understanding ${w} gives students a genuine advantage.`,w=>`${capitalize(w)} makes ${t} more engaging and motivating.`];const conFrames=[w=>`Building ${w} takes considerable time and sustained effort.`,w=>`Without guidance, ${w} can be frustrating to develop.`,w=>`Overemphasis on ${w} may cause other skills to be neglected.`,w=>`${capitalize(w)} levels vary widely and are hard to assess fairly.`,w=>`Not all learners find ${w} easy to acquire naturally.`,w=>`Testing ${w} in formal exams can create unnecessary pressure.`];const pros=items.slice(0,k).map((x,i)=>proFrames[i%proFrames.length](x.word));const cons=items.slice(0,k).map((x,i)=>conFrames[i%conFrames.length](x.word));out.text=`Pros and Cons: ${topic()} (${level()})\n\nPROS:\n${pros.map(p=>`+ ${p}`).join('\n')}\n\nCONS:\n${cons.map(c=>`- ${c}`).join('\n')}\n\nSpeaking task: pick the strongest pro and the strongest con. Debate in pairs: is ${t} worth the time investment?`;const pcCards=[{title:'✅ Pros: arguments for '+topic(),text:pros.map(p=>'+ '+p).join('\n')},{title:'❌ Cons: arguments against '+topic(),text:cons.map(c=>'- '+c).join('\n')},{title:'🗣 Speaking task',text:'Pick the strongest pro and the strongest con.\nDebate in pairs: is '+t+' worth the time investment?\n\nUseful phrases:\n• "The main advantage is…"\n• "However, one major drawback is…"\n• "On balance, I think…"\n• "That\'s a fair point, but…"'}];out.struct={boardKind:'cards',cards:pcCards,items:null,questions:null};}
...
function ttStructListHtml(s,showAns){
  ...
  if(Array.isArray(s.cards)&&s.cards.length){
    ...
    function _hubSectionColor(title){
      ...
      if(/aim|objective|goal|brief|focus/.test(t))return'#4262FF';
      if(/warm|starter|lead.in|hook/.test(t))return'#F59E0B';
      if(/vocab|word|term|glossar/.test(t))return'#EC4899';
      if(/read|input|text|listen|present/.test(t))return'#10B981';
      if(/control|practice|exercise|gap|task [a-c]|[a-d]\.|drill/.test(t))return'#8B5CF6';
      if(/free|produc|speak|discuss|communicat/.test(t))return'#EF4444';
      if(/feed|error|correct|self.check|success/.test(t))return'#6366F1';
      if(/home|assign/.test(t))return'#0EA5E9';
      if(/rule|grammar|structur|form/.test(t))return'#059669';
      if(/example|right|correct/.test(t))return'#16A34A';
      if(/mistake|wrong|common|avoid/.test(t))return'#DC2626';
      if(/pro|advantage|benefit/.test(t))return'#16A34A';
      if(/con|disad|challenge|problem|limit/.test(t))return'#DC2626';
      if(/write|writ|essay|creat/.test(t))return'#8B5CF6';
      if(/key|answer|note|teacher/.test(t))return'#6B7280';
