...
function generate(){if(!activeTool)return;const m=activeTool.mode;const n=count();const src=get('source')||SAMPLE_TEXT;const voc=get('vocab')||SAMPLE_VOCAB;const items=itemsForCount(voc,n);let out={title:activeTool.title,type:m,text:'',cards:[],gameType:activeTool.game||null,gameContent:null,level:level(),tags:[activeTool.cat,topic()]};
  ...
  else if(m==='three-titles'){const tp=topic()||'this topic';const tl=tp.toLowerCase();const kws=extractKeywords(src,6);const k1=kws[0]||tl;const k2=kws[1]||tl;const titleFrames=[`${capitalize(tl)}: Key Ideas and Practical Insights`,`Understanding ${capitalize(tl)}: What You Need to Know`,`${capitalize(k1)} and ${capitalize(k2)}: A Closer Look at ${capitalize(tl)}`,`Why ${capitalize(tl)} Matters: Main Points from the Text`,`${capitalize(tl)}: Challenges, Solutions and Key Takeaways`];const correct=titleFrames[Math.floor(Math.random()*titleFrames.length)];const wrongPool=[`How to Cook a Three-Course Meal on a Budget`,`The History of Ancient Roman Architecture`,`A Guide to Buying Your First Car`,`Tips for Growing Tomatoes in a Small Garden`,`How to Learn Chess in Seven Days`,`The Science of Ocean Tides Explained`,`A Brief History of the Printing Press`,`How to Train for a Marathon in Six Months`];const wrongs=shuffle(wrongPool).slice(0,2);const opts=shuffle([correct,...wrongs]);const ci=opts.indexOf(correct);out.text=`Choose the best title for the text:\n${opts.map((o,i)=>`${String.fromCharCode(65+i)}) ${o}`).join('\n')}\n\nAnswer: ${String.fromCharCode(65+ci)}`;out.gameType='speed-quiz';out.gameContent={questions:[{q:'Choose the best title for the text',opts,correct:ci}]};}
