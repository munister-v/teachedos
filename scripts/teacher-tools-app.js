...
async function generateWithAI(){
  ...
  if(!window._ttAI){toast('AI engine still loading — using fast local generation');return generate();}
  if(!_ttAI.supported()){toast('This browser has no WebGPU — using fast local generation');return generate();}
  const input={level:level(),count:count(),topic:topic(),source:get('source')||'',vocab:get('vocab')||''};
  const body=document.getElementById('result-body');
  body.innerHTML='<div class="result-empty"><div><b>Generating with AI…</b><span id="ai-progress">Loading the language model (first run downloads it once)…</span></div></div>';
  const btn=document.getElementById('ai-gen-btn');if(btn){btn.disabled=true;}
  try{
    const arr=await _ttAI.generate(activeTool.id,input,(t,p)=>{const e=document.getElementById('ai-progress');if(e)e.textContent=t+(p?` ${Math.round(p*100)}%`:'');});
    if(!arr||!arr.length){toast('AI had no result for this tool — using local generation');return generate();}
    const out=aiResultToOutput(activeTool,arr,input);out.aiGenerated=true;
    lastOutput=out;renderResult(out);toast('Generated with AI ✨');
